...
cc.Class({
    ...
    onLoad() {
        ...
        var node_power = this.node.getChildByName("kuang").getChildByName("pow");
        node_power.getComponent(cc.Label).string = this.pow = jiaoben.daojishi.showpower();
        this.schedule(function () {    //倒计时
            var myDate = new Date();
            if (jiaoben.daojishi.showpower() < 8) {
                if (jiaoben.daojishi.kaiguan == true) {
                    this.node.getChildByName("kuang").getChildByName("daojishi").active = true;
                    ...
                    if (myDate.getTime() >= jiaoben.daojishi.showchangetime()) {   //当前时间大于恢复时间   恢复体力
                        ...
                        this.node.getChildByName("kuang").getChildByName("daojishi").active = true;
                        var label_daojishi = this.node.getChildByName("kuang").getChildByName("daojishi").getComponent(cc.Label);
                ...
                this.node.getChildByName("kuang").getChildByName("daojishi").active = false;
            }
        }.bind(this), 1);
    },
